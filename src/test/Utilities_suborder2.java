...
public class Utilities_suborder2 {
	WebDriver driver;
		...
		for (int ar = 0; ar < arrSplit.length; ar++) {
			...
			if (arrSplit[ar].toLowerCase().contains("{number}")) {
				...
				if (subNumberCount == 0) {
					...
					subOrderList = "start";
					System.out.println(
							"start subnumber = //*[@id=\"description\"]/div/div[3]/div[3]/div[2]/" + oltagString);
					oldlistVal = listVal;
					subOrderListNumber++;
					listVal = 1;
...
//									
					...
					System.out.println("xxxxxxxxxxxxxxxxx = " + getAbsoluteXPath(driver.findElement(
